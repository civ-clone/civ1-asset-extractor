...
class LZW {
    ...
    static parseByteStreamToIndexes(binFile, remainingCodedBytes, uByteMode) {
        const parsedIndexes = []; //ArrayList<Integer> parsedIndexes = new ArrayList<Integer>();
        let usableBits = 0, usableBitCount = 0, indicatorLength = 1 /* to increment with ++; rule is that 8+indicatorLength must be <= uByteMode, otherwise reset */, indicatorFlag = 0x001 /* to increment with <<=1 followed by |= 1 */, nextThreshold = 0x0100 /*256*/ /* to increment with <<=1, or *=2 */, decodedCounter = 0, index = 0;
